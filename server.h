...
typedef struct Server{
    ...
    void (*start_listening)(struct Server *server, void *(*request_handler)(char *request)); //the fct which spawns new processes for every incoming request
}server_t;
...
void read_request_message(int fd, char *buffer) //reads at most BYTES_SIZE_IN_LISTEN from the given fd and fills the buffer
#endif