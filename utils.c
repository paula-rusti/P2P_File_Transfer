struct offset *segment_file(char *file_name, unsigned int nr_of_peers){
    ...
    nr_bytes = (unsigned int) sb.st_size / nr_of_peers;//get the neccesary bytes to be transferred by every peer
    ...
    for(int index = 0; index < nr_of_peers; index++)
    {
        ...
        if (aux == 0){
            ...
            temp.end = nr_bytes - 1;//because we start from 0
        }
        else{
            temp.start = aux + 1;//else is end + 1
            temp.end = temp.start + nr_bytes;
        }
        aux = temp.end;
